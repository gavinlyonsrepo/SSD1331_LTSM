/*!
	@file     SSD1331_LTSM.cpp
	@author   Gavin Lyons
	@brief    Source header file for SSD1331_LTSM OLED library.
*/

#include "SSD1331_LTSM.hpp"


/*! @brief Constructor for class SSD1331_LTSM
 ...
 * */
SSD1331_LTSM::SSD1331_LTSM(color_order_e colororder, const Constrast_values_t& contrast, const Dim_Constrast_values_t& dimContrast)
...
*/
void  SSD1331_LTSM ::PowerDown(void)
{
	sleepDisplay();
...
*/
void SSD1331_LTSM::SetupGPIO_SPI(uint16_t CommDelay, int8_t rst, int8_t dc, int8_t cs, int8_t sclk, int8_t din)
{
...
*/
void SSD1331_LTSM::SetupGPIO_SPI(uint32_t speed_hz, int8_t rst, int8_t dc, int8_t cs)
{
...
*/
void SSD1331_LTSM::SSD1331Initialize() {
	ResetPin();
...
*/
void SSD1331_LTSM::HWSPIInitialize(void)
{
	SPI.begin();
}
...
*/
void SSD1331_LTSM::ScrollSetup(uint8_t horOffset,uint8_t startRowAddr, uint8_t numOfRows, uint8_t verOffset, scroll_time_interval_e scrollTimeInterval) {
	// Check user input
	...
	if (startRowAddr + numOfRows > 64) {
			...
			Serial.print(F("ScrollSetup: startRowAddr + numOfRows > 64, setting numOfRows = 1, "));
			Serial.print(startRowAddr);
			Serial.print(F(" + "));
			Serial.print(numOfRows);
			Serial.println(F(" <= 64"));
		#endif
		numOfRows = 1;
	}
...
*/
void SSD1331_LTSM::Scroll(scroll_control_e scrollActivate)  {
...
*/
void SSD1331_LTSM::changeDisplayMode(display_mode_e mode) {
	switch (mode)
	{
...
/*!
	...
	@param mode OLED_rotate_e enum
		...
		3 =  270 rotate
*/
void SSD1331_LTSM::setRotation(display_rotate_e mode) {
...
*/
void SSD1331_LTSM::InitScreenSize(uint16_t width_OLED, uint16_t height_OLED)
{
	_width = width_OLED;
	_height = height_OLED;
...
*/
uint16_t SSD1331_LTSM::HighFreqDelayGet(void) const {return _SWSPIGPIODelay;}

/*!
	@brief Freq delay used in SW SPI setter, uS delay used in SW SPI method
	@param CommDelay The GPIO communications delay in uS
*/
void  SSD1331_LTSM::HighFreqDelaySet(uint16_t CommDelay){_SWSPIGPIODelay = CommDelay;}
...
*/
void SSD1331_LTSM::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
...
*/
void SSD1331_LTSM::cmdBegin(void)
{
	writeCommand(_CMD_DISPLAYOFF);
	ConfigueContrast();
...
*/
void SSD1331_LTSM::ConfigueDimMode(void)
{
...
*/
void SSD1331_LTSM::ConfigueContrast(void)
{
 ...
 */
void SSD1331_LTSM::ClearWindowCmd(uint8_t column1 , uint8_t row1 , uint8_t column2 , uint8_t row2)
{
	if (_displayRotate % 2 == 1) // 90° or 270° rotations,
	{
		swap_uint8(column1, row1);
		swap_uint8(column2, row2);
	}
 ...
 */
void SSD1331_LTSM::CopyWindowCmd(uint8_t column1 , uint8_t row1 , uint8_t column2 , uint8_t row2, uint8_t column3 , uint8_t row3)
{
	if (_displayRotate % 2 == 1) // 90° or 270° rotations,
	{
		swap_uint8(column1, row1);
		swap_uint8(column2, row2);
		swap_uint8(column3, row3);
	}
 ...
 */
void SSD1331_LTSM::DimWindowCmd(uint8_t column1 , uint8_t row1 , uint8_t column2 , uint8_t row2)
{
	if (_displayRotate % 2 == 1) // 90° or 270° rotations,
	{
		swap_uint8(column1, row1);
 ...
 */
void SSD1331_LTSM::DrawLineCmd(uint8_t c1, uint8_t r1, uint8_t c2, uint8_t r2, uint16_t color)
{
	if (_displayRotate % 2 == 1) // 90° or 270° rotations,
	{
		swap_uint8(c1, r1);
...
/*!
    ...
    @note this uses built-in OLED command NOT the graphics library.
 */
void SSD1331_LTSM::DrawRectCmd(uint8_t c1, uint8_t r1, uint8_t  c2, uint8_t r2, uint16_t color, bool fill) {
 ...
 */
void SSD1331_LTSM::WriteColor16(uint16_t colorWrite)
{
